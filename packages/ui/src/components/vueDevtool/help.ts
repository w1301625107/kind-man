// @ts-nocheck
// https://github.com/vuejs/devtools/blob/7012ebd843/packages/app-backend-core/src/hook.ts

function detect() {
  if (window.__VUE_DEVTOOLS_GLOBAL_HOOK__ == undefined) {
    ...
    function clone(object, options = null) {
      ...
      const handleCopy = (object, cache) => {
        ...
        if (
          // promise-like
          (hasOwnProperty.call(object, "then") &&
  ...
  if (window.__VUE_DEVTOOLS_HOOK_REPLAY__) {
    try {
      window.__VUE_DEVTOOLS_HOOK_REPLAY__.forEach((cb) => cb(hook))
      window.__VUE_DEVTOOLS_HOOK_REPLAY__ = []
    } catch (e) {
